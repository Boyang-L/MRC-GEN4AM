...
class SciArgProcessor(DataProcessor):
    ...
    def convert_examples_to_features(self, examples, tokenizer, max_sen_num,
        ...
        for example in examples:
            ...
            for i, (acc, arc) in enumerate(zip(example.acc, example.arc)):
                labels_acc[i] = self.ac_label_map[acc]
            ...
            if query_idx == -1:
                ...
                if args.graph_rep_type == "path":
                    ...
                    if example.graph_rep == []:
                        graph_label = "<graph> "

            if not args.with_graph:
                graph_label = "<graph> "
            labels_arc = [0] * max_sen_num
            labels_ard_list = []
            labels_ard_ts = []

            if example.answer:
                for l in example.answer:
                    labels_ard_list.append(special_tokens[l[1]-1])
                labels_ard_ts.append(" ".join(st for st in labels_ard_list))

                if len(labels_ard_list) > 1 and not evaluate and args.data_aug == "td":
                    labels_ard_list.reverse()
                    labels_ard_ts.append(" ".join(st for st in labels_ard_list))

                for labels_ard_t in labels_ard_ts:
                    labels_ard_t = graph_label + "<answer> " + labels_ard_t

                    labels_ard_ori = tokenizer(labels_ard_t, max_length=50, return_tensors="pt", pad_to_max_length=True)["input_ids"]
                    labels_ard_ids = labels_ard_ori[:, :-1].contiguous()
                    labels_ard = labels_ard_ori[:, 1:].clone()
                    labels_ard[labels_ard_ori[:, 1:] == pad_id] = -100
                    graph_len = torch.where(labels_ard[0] == answer_ids)[0].tolist()[0]
                    graph_atten = torch.cat((torch.ones(graph_len), torch.zeros(labels_ard.size()[1] - graph_len)))
                    ...
                    features.append(
                        InputFeatures(input_ids=src_tokens_idxs.tolist(),
                                      ...
                                      decoder_input_ids=labels_ard_ids[0].tolist(),
                                      acc_label_ids=labels_acc,
                                      ard_label_ids=labels_ard[0].tolist(),
                ...
                labels_ard_t = graph_label[:-1]

                labels_ard_ori = tokenizer(labels_ard_t, max_length=50, return_tensors="pt", pad_to_max_length=True)["input_ids"]
                labels_ard_ids = labels_ard_ori[:, :-1].contiguous()
                labels_ard = labels_ard_ori[:, 1:].clone()
                labels_ard[labels_ard_ori[:, 1:] == pad_id] = -100
                graph_len = len(labels_ard_t.split(" "))
                graph_atten = torch.cat((torch.ones(graph_len), torch.zeros(labels_ard.size()[1] - graph_len)))
                labels_atten = 1 - graph_atten
                graph_atten = graph_atten.long()
                labels_atten = labels_atten.long()
