# This code is adopted from https://github.com/HLT-HITSZ/trans_am
# This code provides the argument markers used in "A Neural Transition-based Model for Argumentation Mining"
# We also leverage the argument markers in our model
# We added AR type into the output file
...
def get_essay_detail(essay_lines, max_n_spans):
    ...
    essay_id = int(essay_lines[0].split("\t")[0])
    # index, list of (start_idx, end_idx) of each ac span
    ac_spans = []
    # text of each span
    ac_texts = []
    ...
    ac_relations = []
    # list of (startr_idx, end_idx) of each am（marker） span
    shell_spans = []
    ...
    text = [line.strip().split("\t")[-1] for line in essay_lines]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path", type=str, default='./data/PE_token_level_data.tsv')
    parser.add_argument("--data-save-path", type=str, default='./data/PE_data_df.csv')
    ...
    for global_para_id, para_info_dict in essay_info_dict_list.items():
        ...
        for child, parent in enumerate(ac_rel_targets):
            if parent != 12:
                ...
                AR_types.append(id2rel[ar_rel_types[child]])
        df_dict["AR_pairs"].append(AR_pairs)
        df_dict["AR_types"].append(AR_types)
