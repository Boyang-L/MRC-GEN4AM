...
class InputExample(object):
    def __init__(self, doc_id, context, query, answer=None, ac_pos=None, arc=None, acc=None, graph_rep=None):
...
class SciArgProcessor(DataProcessor):
    ...
    def __init__(self):
        self.ac_labels = ['none', 'Claim', 'Premise']
        self.ar_labels = ['support', 'attack']
    ...
    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self.read_sci_data(os.path.join(data_dir, "train.csv")), "train")

    def get_dev_examples(self, data_dir):
        return self._create_examples(self.read_sci_data(os.path.join(data_dir, "dev.csv")), "dev")

    def get_test_examples(self, data_dir):
        return self._create_examples(self.read_sci_data(os.path.join(data_dir, "test.csv")), "test")
    ...
    def read_sci_data(cls, input_file):
        ...
        with open(input_file, "r") as f:
            ...
            for line in reader:
                ...
                if doc_id != doc_id_pre:
                    if doc_id_pre != 0:
                        ...
                        for k in data['adu_type'].keys():
                            ...
                            data['path'][k] = get_path(data['argraph'], int(data['root_num']), data['all_roots'], int(k), data['arc'])
                ...
                if int(line['root']) == 2 or int(line['root']) == 1:
    ...
    def _create_examples(self, dataset, data_type="train"):
        examples = []

        for data in dataset:
            doc_id = data['doc_id']
            context = data['context']
            query = 'What is the main purpose of this paper'
            answer = data['root']
            examples.append(
                InputExample(doc_id, context, query, answer=answer, ac_pos=data['ac_pos'], arc=data['arc'], acc=data['acc']))
            for k in data['qapairs'].keys():
                query = data['adu_type'][str(k)][1]
                answer = data['qapairs'][k]

                examples.append(
                    InputExample(doc_id, context, query, answer=answer, ac_pos=data['ac_pos'], arc=data['arc'], acc=data['acc'], graph_rep=data[args.graph_rep_type][str(k)]))
    ...
    def convert_examples_to_features(self, examples, tokenizer, max_sen_num,
        ...
        for s_i in range(max_sen_num):
            special_tokens.append("<support_AC" + str(s_i) + ">")
            special_tokens.append("<attack_AC" + str(s_i) + ">")
        ...
        for example in examples:
            ...
            if example.query == "What is the main purpose of this paper":
                query_idx = -1
                continue
            else:
                query_idx = example.context.index(example.query)
                query_idx = example.ac_pos.index(query_idx)
            query = query_token + ": " + example.query
            src_tokens = src_tokens + query + context_token + ":"

            for i_c, context in enumerate(example.context):
                if i_c in example.ac_pos:
                    src_tokens = src_tokens + " " + special_tokens[example.ac_pos.index(i_c)] + " " + context
                else:
                    src_tokens = src_tokens + " " + "<non_AC>" + " " + context
            ...
            labels_ard = [0] * max_sen_num
            labels_acc = [0] * max_sen_num
            ...
            if query_idx == -1:
                ...
                if args.graph_rep_type == "path":
                    graph_label = "<graph> " + " ".join(gr for gr in example.graph_rep) + " "
                else:
                    for gr in example.graph_rep:
                        graph_label = graph_label + " ".join(gn for gn in gr) + " | "
            ...
            if example.answer:
                # max num of label(sentence)
                for l in example.answer:

                    labels_arc_list.append("<" + l[0].split(':')[1] + ">" + " " + special_tokens[l[1]-1])
                labels_arc_ts.append(" ".join(st for st in labels_arc_list))

                if len(labels_arc_list) > 1 and not evaluate and args.data_aug == "td":
                    labels_arc_list.reverse()
                    labels_arc_ts.append(" ".join(st for st in labels_arc_list))

                for labels_arc_t in labels_arc_ts:
                    labels_arc_t = graph_label + "<answer> " + labels_arc_t

                    labels_arc_ori = tokenizer(labels_arc_t, max_length=100, return_tensors="pt", pad_to_max_length=True)["input_ids"]
                ...
                labels_arc_t = graph_label + "<answer>"
...
def get_path(old_edges, root, all_roots, node, arc_label):
    edges = []
            ...
            if p > -1:
                for n in edges[p]:
                    if n not in path_nodes:
                        path.append("<" + arc_label[n] + "> <AC" + str(n) + ">")
                        path_nodes.append(n)
    if path == []:
        path = ["<" + arc_label[node] + "> <AC" + str(node) + ">"]
    return path
