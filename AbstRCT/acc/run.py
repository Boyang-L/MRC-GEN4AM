...
from utils.metrics import get_pairs, compute_acc
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        with torch.no_grad():
            ...
            target = [tokenizer.decode(t, skip_special_tokens=True, clean_up_tokenization_spaces=True) for t in batch[4]]
            predictions.extend(preds)
            actuals.extend(target)
            queries.extend(batch[7].tolist())
            doc_ids.extend(batch[8].tolist())
    preds = get_pairs(doc_ids, queries, predictions)
    result = compute_acc(preds)
